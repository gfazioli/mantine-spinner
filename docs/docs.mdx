...
The `size` prop accepts a Mantine breakpoint object (`{ base, xs, sm, md, lg, xl }`) and resizes the spinner via CSS media queries — **no React re-renders** on viewport changes. The internal SVG geometry is computed at the base breakpoint value; the rendered width and height are scaled through the `--spinner-size` CSS variable, so stroke proportions stay visually correct across all sizes.
